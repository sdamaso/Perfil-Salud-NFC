...
final class NFCDatasource{
    ...
    //
    //
    func getNFCProfilesAUX(nfcUid: String, completionBlock: @escaping (Result<NFCModel, Error>) -> Void){
        let docRef = database.collection(collection).document(nfcUid)
        docRef.getDocument(as: NFCModel.self) {result in
            switch result{
            case .success(let auxNFC):
                completionBlock(.success(auxNFC))
            case .failure(let error):
                print(error.localizedDescription)
                completionBlock(.failure(error))
            }
